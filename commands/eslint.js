...
const generatePackage = async function (pkj, isTs) {
    ...
    const devDependenciesNeedAdd = ['eslint', 'eslint-config-tongdun', 'eslint-plugin-td-rules-plugin', 'lint-staged', 'husky'].concat([
        ...tsArr
    ]);
    ...
    prompt(question)
        .then(async ({ type, isTs }) => {
            ...
            if (hasHusky) {
                ...
                // 复制.husky目录过去
                await shell.cp('-R', [path.resolve(__dirname, '../template/husky/.husky')], process.cwd());
            }
