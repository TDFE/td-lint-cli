...
const allFiles = fg.sync([`${process.cwd()}/src/**/*.*`]);
        ...
        webpack(
            ...
            (err, stats) => {
                ...
                if (stats.hasWarnings()) {
                    const warns = Array.isArray(info.warnings)
                        ? info.warnings.map((i) => {
                              return i.message && i.message.replace(`${process.cwd()}/`, '');
                          })
                        : [];
                    const str = warns.join('\n');
                    console.warn(str);
                    spinner.succeed(`完成查找,共计${warns.length}个`);
                    return;
                }
